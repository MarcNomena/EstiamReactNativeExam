...
import { LocalisationArray } from "@/interface/MapInteractor";
 ...
 const ProductScreen = () => {
  //  const [localisation, setLocalisation] = useState<Localisation[]>([]);
    const [localisationArray, setLocalisationArray] = useState<LocalisationArray>();
    const { getData } = useDatabase();
      useEffect(() => {
        const useGetData = async () => {
          try{
            setLocalisationArray(await getData());
          }catch(e){
    ...
    return (
        <View style={styles.container}>
            <Text>Localise Me ESTIAM in France</Text>


            <View>
           {localisationArray?.localisation ? <MapComponent localisation={localisationArray?.localisation}/>: <Button title="Solid" type="solid" loading />}
            </View>
